...
public class PSODEPath {
	public static int n;
	public double[] angles;
	public Point[] points;
	
	public PSODEPath(int no) {
		n = no;
		angles = new double[n];
		points = new Point[n];
	}
	...
	public void convertAngles() {
		for (int i=0; i < n; i++) {
			points[i] = PSODEPath.convertAtoP(angles[i], PSODE.R, PSODE.startPoint, PSODE.endPoint);
		}
	}
	...
	public double pathDistance(double R) {
		...
		a2 = Math.pow(Math.abs(PSODE.startPoint.x - PSODE.endPoint.x), 2);
	    b2 = Math.pow(Math.abs(PSODE.startPoint.y - PSODE.endPoint.y), 2);
	...
	public double pathDistancePoint() {
		...
		dis += Math.sqrt(Math.pow(points[n-1].x - PSODE.endPoint.x, 2) + Math.pow(points[n-1].y-PSODE.endPoint.y, 2));
		return dis;
	}
